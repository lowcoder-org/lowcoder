...
import { ActionConfig, ActionCategory, ActionExecuteParams } from "./types";
import { 
    multiChangeAction, 
    wrapActionExtraInfo, 
    changeValueAction, 
...
const addComponentAction: ActionConfig = {
  key: 'add-components',
  ...
  execute: async (params: ActionExecuteParams) => {
    ...
    try {
      ...
      // Get component manifest and default data
      ...
      // Get current layout and calculate position
...
const moveComponentAction: ActionConfig = {
  ...
  validation: (value: string) => {
    ...
    // Parse the input to validate format
  ...
  execute: async (params: ActionExecuteParams) => {
    ...
    try {
      ...
      if (!container) {
        message.error('No container available');
        return;
      }
      ...
      // Dispatch the action to update the layout
      simpleContainer.dispatch(
        wrapActionExtraInfo(
          multiChangeAction({
...
const deleteComponentAction: ActionConfig = {
  ...
  execute: async (params: ActionExecuteParams) => {
    ...
    try {
      ...
      // Create new layout without the component
      const newLayout = { ...currentLayout };
      delete newLayout[componentKey];

      // Dispatch the action to remove the component
      simpleContainer.dispatch(
        wrapActionExtraInfo(
          multiChangeAction({
            layout: changeValueAction(newLayout, true),
...
const resizeComponentAction: ActionConfig = {
...
const configureComponentAction: ActionConfig = {
...
// Layout Actions
const changeLayoutAction: ActionConfig = {
  key: 'change-layout',
  label: 'Change layout',
  category: 'layout',
  requiresInput: true,
...
// Event Actions
const addEventHandlerAction: ActionConfig = {
...
// Style Actions
const applyStyleAction: ActionConfig = {
  key: 'apply-style',
...
export const actionCategories: ActionCategory[] = [
  {
    ...
    actions: [addComponentAction, moveComponentAction, deleteComponentAction, resizeComponentAction]
  },
  {
...
export const actionRegistry = new Map<string, ActionConfig>();
