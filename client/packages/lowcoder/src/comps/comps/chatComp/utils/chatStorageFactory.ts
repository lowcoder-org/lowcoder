// client/packages/lowcoder/src/comps/comps/chatComp/utils/chatStorageFactory.ts
...
export const createChatStorage = (tableName: string) => {
  ...
  return {
    ...
    async saveThread(thread: ThreadData) {
      try {
        // Insert or replace thread
        await alasql.promise(`
          DELETE FROM ${threadsTable} WHERE threadId = ?
        `, [thread.threadId]);
    ...
    async saveMessage(message: MyMessage, threadId: string) {
      try {
    ...
    async saveMessages(messages: MyMessage[], threadId: string) {
      try {
