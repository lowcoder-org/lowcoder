...
const AvatarGroupView = (
  ...
  const getDefaultValueMS = useMemo(() => {
    const [HH, MM, SS, MS] =
      (props.defaultValue.value && props.defaultValue.value.split(':')) || [];
    let totleMS =
  ...
  const startTimer = () => {
    setIntervalId(
      Number(
        setInterval(() => {
          let temp =
            props.timerType === 'timer'
              ? Date.now() - startTime
              : getDefaultValueMS - (Date.now() - startTime);
          if (temp <= 0) {
  ...
  const startAction = () => {
    ...
    props.onEvent('start');
    setStartTime(
      props.timerType === 'timer' ? Date.now() - getDefaultValueMS : Date.now()
    );
  };
  ...
  const resumeAction = () => {
    setStartTime(
      props.timerType === 'timer'
        ...
        : Date.now() - (getDefaultValueMS - elapsedTime)
    );
    setTimerState('started');
...
let AvatarGroupBasicComp = (function () {
  return new UICompBuilder(childrenMap, (props, dispatch) => (
    <AvatarGroupView {...props} dispatch={dispatch} />
  ))
    ...
    .setExposeMethodConfigs([
      {
        ...
        execute: async (comp, params) => {
          if (comp.children.timerState.value === 'stoped')
            comp.children.actionHandler.dispatch(
              comp.children.actionHandler.changeValueAction('start')
            );
          else if (comp.children.timerState.value === 'paused')
            comp.children.actionHandler.dispatch(
              comp.children.actionHandler.changeValueAction('resume')
            );
          else if (comp.children.timerState.value === 'started')
            comp.children.actionHandler.dispatch(
              comp.children.actionHandler.changeValueAction('pause')
            );
        },
      },
      {
        ...
        execute: async (comp, params) => {
          comp.children.actionHandler.dispatch(
            comp.children.actionHandler.changeValueAction('reset')
          );
        },
      },
    ])
