import { e as I, F as yt, G as ft, s as dt, a as pt, d as gt, b as mt, H as xt, j as G, k as kt } from "./08856db2.js";
import { d as _t, f as bt, a as vt, g as it } from "./0fddf2f2.js";
import { a as Q } from "./eb460319.js";
import "./256b619e.js";
var W = function() {
...
const wt = W;
let C = "";
const X = [], V = [], F = [], $t = function() {
  X.length = 0, V.length = 0, C = "", F.length = 0, xt();
}, Tt = function(t) {
  C = t, X.push(t);
}, Mt = function() {
  return X;
...
const j = I().journey, P = j.leftMargin, Ot = function(t, e, i, l) {
  ...
  let c;
  h === "sandbox" && (c = G("#i" + e));
  const f = h === "sandbox" ? G(c.nodes()[0].contentDocument.body) : G("body");
  v.init();
  const y = f.select("#" + e);
  L.initGraphics(y);
...
}, H = j.sectionFills, st = j.sectionColours, qt = function(t, e, i) {
  const l = I().journey;
  let n = "";
  ...
  for (const [p, s] of e.entries()) {
    if (n !== s.section) {
      y = H[f % H.length], g = f % H.length, m = st[f % st.length];
      let a = 0;
      const u = s.section;
