...
import {
  GOOGLE_FORMS_PROXY_PREFIX,
  buildGoogleFormsProxiedUrlFromRequest,
} from "./googleUrls";
import {
  createGoogleFormsProxySession,
  joinGoogleFormsProxySession,
} from "./googleSession";
...
const ALLOWED_GOOGLE_FORMS_HOSTS = new Set(
  (process.env.LOWCODER_GOOGLE_FORMS_ALLOWED_HOSTS ?? "docs.google.com")
    .split(",")
...
export function registerGoogleFormsProxy(app: Express): void {
  app.get(BRIDGE_PATH, (_req, res) => {
    const bridgePath = path.join(__dirname, "bridge", "google-forms-bridge.js");
  ...
  router.post("/session", express.json({ limit: "1mb" }), createGoogleFormsProxySession);
  router.post("/session/join", express.json({ limit: "1mb" }), joinGoogleFormsProxySession);

  router.use(express.raw({ type: "*/*", limit: "25mb" }), async (req, res) => {
    if (req.path === "/session" || req.path === "/session/join") {
      res.status(405).json({
        message:
          "Use POST on /proxy/google-forms/session or /proxy/google-forms/session/join",
      });
      return;
    }
    ...
    try {
      ...
      res.status(502).json({ message: "Google Forms proxy request failed" });
    }
  });

  app.use(GOOGLE_FORMS_PROXY_PREFIX, router);
}
...
function resolveUpstreamUrl(req: Request): URL {
  ...
  assertAllowedGoogleFormsUrl(upstream);
  return upstream;
}

function assertAllowedGoogleFormsUrl(url: URL): void {
  if (
    url.protocol !== "https:" ||
    !ALLOWED_GOOGLE_FORMS_HOSTS.has(url.hostname.toLowerCase()) ||
...
function relayHeaders(upstreamResponse: FetchResponse, res: Response, req: Request): void {
  ...
  return rawValue
    ...
    .map((cookie) =>
      cookie
        ...
        .replace(/;\s*Path=[^;]+/gi, `; Path=${GOOGLE_FORMS_PROXY_PREFIX}`)
    );
}
...
function rewriteBodyUrls(body: string, req: Request, upstreamUrl: URL): string {
  ...
  let output = body.replace(
    ...
    (_match, suffix: string) =>
      buildGoogleFormsProxiedUrlFromRequest(`${origin}${suffix ?? ""}`, req)
  );

  output = output.replace(
    /(href|src|action)=["'](\/forms\/[^"']*)["']/gi,
    (_match, attr: string, route: string) =>
      `${attr}="${buildGoogleFormsProxiedUrlFromRequest(`${origin}${route}`, req)}"`
  );
  output = output.replace(
    /url\(["']?(\/forms\/[^"')]+)["']?\)/gi,
    (_match, route: string) =>
      `url("${buildGoogleFormsProxiedUrlFromRequest(`${origin}${route}`, req)}")`
  );
  output = output.replace(
    /(["'])(\/forms\/[^"'\\\s]*)\1/g,
    (_match, quote: string, route: string) =>
      `${quote}${buildGoogleFormsProxiedUrlFromRequest(`${origin}${route}`, req)}${quote}`
  );
  return output;
}
...
function rewriteAbsoluteUrl(value: string, req: Request): string {
  try {
    const parsed = new URL(value, "https://docs.google.com");
    assertAllowedGoogleFormsUrl(parsed);
    return buildGoogleFormsProxiedUrlFromRequest(parsed.toString(), req);
  } catch {
    return value;
  }
}
...
function isAuthorized(req: Request): boolean {
  ...
  return verifyProxyToken(token, "google-forms-proxy");
}
