...
${err.toString()}`);
  ...
  // src/bridge/pointer-presence/PointerOverlay.ts
  var PointerOverlay = class {
    constructor() {
    ...
    setShowRemoteCursors(show) {
      ...
      if (!show) {
        for (const id2 of Array.from(this.cursors.keys())) {
          this.removeCursor(id2);
        }
      }
    }
    syncFromStates(states) {
      ...
      for (const state of states) {
        const id2 = state.user.id;
        if (this.showRemoteCursors && state.pointer) {
          seenCursors.add(id2);
          this.upsertCursor(id2, state.user.name, state.user.color, state.pointer);
        }
        if (state.selection && state.selection.rects.length > 0) {
          seenSelections.add(id2);
          this.upsertSelection(id2, state.user.name, state.user.color, state.selection);
        }
      }
      for (const id2 of Array.from(this.cursors.keys())) {
        if (!seenCursors.has(id2)) this.removeCursor(id2);
      }
      for (const id2 of Array.from(this.selections.keys())) {
        if (!seenSelections.has(id2)) this.removeSelection(id2);
      }
    }
    showClickRipple(xRatio, yRatio, color = "#1E88E5") {
      if (this.destroyed) return;
    ...
    upsertCursor(id2, name, color, pointer) {
      let el = this.cursors.get(id2);
      if (!el) {
        el = document.createElement("div");
        el.innerHTML = `<svg width="18" height="22" viewBox="0 0 18 22" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M1 1L16.5 10.5L9.5 12.5L6.5 20.5L1 1Z" fill="${color}" stroke="#fff" stroke-width="1.2"/></svg><span style="display:inline-block;margin-left:2px;margin-top:10px;padding:1px 6px;border-radius:4px;background:${color};color:#fff;font:11px/16px system-ui,sans-serif;white-space:nowrap;box-shadow:0 1px 2px rgba(0,0,0,.25)">${escapeHtml(name)}</span>`;
    ...
    upsertSelection(id2, name, color, selection) {
      let group = this.selections.get(id2);
      if (!group) {
        group = document.createElement("div");
        group.dataset.selectionUser = id2;
    ...
    removeCursor(id2) {
      const el = this.cursors.get(id2);
      if (!el) return;
      el.remove();
      this.cursors.delete(id2);
    }
    removeSelection(id2) {
      const el = this.selections.get(id2);
      if (!el) return;
      el.remove();
      this.selections.delete(id2);
    }
  };
  function clamp(value, min4, max4) {
    return Math.min(max4, Math.max(min4, value));
  }
  function escapeHtml(value) {
    return value.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;");
  }

  // src/bridge/pointer-presence/PointerPresenceProvider.ts
  var PointerPresenceProvider = class {
    constructor(provider, user, onRemoteChange, throttleMs = 33) {
  ...
  // src/bridge/pointer-presence/initPointerPresence.ts
  var SHOW_MICE_STORAGE_KEY = "lowcoder-website-show-remote-mice";
  function initPointerPresence(config) {
    const username = config.username || new URLSearchParams(window.location.search).get("username") || document.documentElement.getAttribute("data-lowcoder-username") || config.editorId;
    const user = {
      id: config.editorId,
      name: username,
    ...
    return {
      showClickRipple: (xRatio, yRatio, color) => overlay.showClickRipple(xRatio, yRatio, color ?? user.color),
      showButtonClickFlash: (rect, color, label) => overlay.showButtonClickFlash(rect, color, label),
      destroy: () => {
  ...
  function createMiceToggleChrome(initial, onChange) {
    ...
    text2.textContent = "Show others' mice";
  ...
  function readShowMicePreference() {
    try {
  ...
  function writeShowMicePreference(show) {
    try {
  ...
  function clientRectsToRatios(clientRects) {
  ...
  (() => {
    ...
    const username = params2.get("username") || root.dataset.lowcoderUsername || editorId;
    ...
    if (!roomId || !collabId) {
      console.error(
        "[website-bridge] Missing roomId/collab. Load the page through /proxy/website and create a session with createWebsiteProxySession before Explore Together."
      );
      return;
    }
    ...
    let scrollPublishTimer;
    let fieldPublishTimer;
    let lastPublishedScroll = "";
    let dismissedNavSeq = 0;
    let followPromptEl = null;
    const doc2 = new Doc();
    ...
    function log(...args2) {
      if (debug) console.log("[website-bridge]", role, ...args2);
    }
    ...
    function controlIdentity(control) {
      ...
      const placeholder = control instanceof HTMLInputElement || control instanceof HTMLTextAreaElement ? control.placeholder?.trim() : "";
    ...
    function schedulePublishControl(control) {
    ...
    function applyScroll(scroll) {
      ...
      try {
        window.scrollTo({
          left: clamp2(scroll.xRatio, 0, 1) * maxX,
          top: clamp2(scroll.yRatio, 0, 1) * maxY,
          behavior: "auto"
        });
      } finally {
        window.setTimeout(() => {
    ...
    function publishClick(event) {
      ...
      if (button) {
        ...
        const label = (button.getAttribute("aria-label") || button.textContent || (button instanceof HTMLInputElement ? button.value : "") || "Button").trim().replace(/\s+/g, " ").slice(0, 80);
        click = {
          ...click,
          kind: "button",
          label,
    ...
    function hideFollowPrompt() {
      followPromptEl?.remove();
      followPromptEl = null;
    }
    function showFollowPrompt(offer) {
    ...
    function applyRemoteClick(click) {
      ...
      if (!isPresenter) {
        ...
        if (scrollRaw) {
          try {
            ...
            if (Math.abs(local.xRatio - scroll.xRatio) > 0.01 || Math.abs(local.yRatio - scroll.yRatio) > 0.01) {
              applyScroll(scroll);
    ...
    document.addEventListener(
      ...
      (event) => {
        ...
        const isFormControl = formControl instanceof HTMLInputElement || formControl instanceof HTMLTextAreaElement || formControl instanceof HTMLSelectElement;
        if (!isFormControl) {
          publishClick(event);
        } else if (formControl instanceof HTMLInputElement && ["checkbox", "radio"].includes(formControl.type)) {
    ...
    history.pushState = function(...args2) {
      const result = originalPushState(...args2);
      onHistoryChange();
      return result;
    };
    history.replaceState = function(...args2) {
      const result = originalReplaceState(...args2);
      onHistoryChange();
      return result;
    };
