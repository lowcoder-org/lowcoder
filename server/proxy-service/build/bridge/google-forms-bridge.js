...
${err.toString()}`);
  ...
  // src/bridge/google-forms/proxyRuntime.ts
  var URL_ATTRIBUTES = ["src", "href", "action", "data-src", "poster"];
  var SKIPPED_LINK_RELS = /* @__PURE__ */ new Set(["preconnect", "dns-prefetch"]);
  function installGoogleFormsProxyRuntime() {
    const config = window.__LOWCODER_GOOGLE_PROXY__;
    if (!config || window.__LOWCODER_GOOGLE_PROXY_READY__) return;
    window.__LOWCODER_GOOGLE_PROXY_READY__ = true;
    let upstreamBase;
    try {
      upstreamBase = new URL(config.upstreamUrl);
    } catch {
      return;
    }
    const rootPaths = new Set(config.rootPaths);
    function isGoogleHost(hostname) {
      const host = hostname.toLowerCase();
      return config.hosts.some(
        (allowed) => allowed.startsWith(".") ? host.endsWith(allowed) : host === allowed
      );
    }
    function isGoogleRootPath(pathname) {
      const segment = pathname.split("/")[1] ?? "";
      return rootPaths.has(segment);
    }
    function toProxiedUrl(absoluteUrl) {
      const params2 = new URLSearchParams();
      params2.set("target", absoluteUrl);
      Object.entries(config.params).forEach(([key, value]) => {
        if (value) params2.set(key, value);
      });
      return `${config.prefix}?${params2.toString()}`;
    }
    function rewrite(rawUrl) {
      if (!rawUrl || /^(data|blob|javascript|about|mailto|tel):/i.test(rawUrl)) return null;
      let resolved;
      try {
      ...
      if (resolved.pathname.startsWith(config.prefix)) return null;
      if (resolved.protocol !== "http:" && resolved.protocol !== "https:") return null;
      if (resolved.origin === window.location.origin) {
        if (!isGoogleRootPath(resolved.pathname)) return null;
        const upstream = new URL(
          `${resolved.pathname}${resolved.search}${resolved.hash}`,
          upstreamBase
        );
        return toProxiedUrl(upstream.toString());
      }
      if (!isGoogleHost(resolved.hostname)) return null;
      resolved.protocol = "https:";
      return toProxiedUrl(resolved.toString());
    }
    patchFetch(rewrite);
    patchXhr(rewrite);
    patchSendBeacon(rewrite);
    patchElementUrls(rewrite);
  }
  function patchFetch(rewrite) {
    ...
    window.fetch = ((input, init) => {
      try {
        if (typeof input === "string" || input instanceof URL) {
          ...
          if (next) return originalFetch(new Request(next, input), init);
        }
      } catch {
      }
  ...
  function patchXhr(rewrite) {
    const originalOpen = XMLHttpRequest.prototype.open;
    XMLHttpRequest.prototype.open = function open(method, url, ...rest) {
      let target = url;
      try {
        target = rewrite(String(url)) ?? url;
      } catch {
        target = url;
      }
  ...
  function patchSendBeacon(rewrite) {
    const originalSendBeacon = navigator.sendBeacon?.bind(navigator);
    if (!originalSendBeacon) return;
    navigator.sendBeacon = ((url, data) => {
      try {
        const next = rewrite(String(url));
        if (next) return originalSendBeacon(next, data);
      } catch {
      }
      return originalSendBeacon(url, data);
    });
  }
  function patchElementUrls(rewrite) {
    function shouldSkip(element2) {
      if (!(element2 instanceof HTMLLinkElement)) return false;
      const rel = (element2.getAttribute("rel") || "").toLowerCase();
      return SKIPPED_LINK_RELS.has(rel);
    }
    function patchProperty(prototype, property) {
      ...
      Object.defineProperty(prototype, property, {
        ...
        set(value) {
          let next = null;
          try {
            if (!shouldSkip(this)) next = rewrite(String(value));
          } catch {
            next = null;
          }
          descriptor.set.call(this, next ?? value);
        }
      });
    }
    ...
    Element.prototype.setAttribute = function setAttribute(name, value) {
      let next = null;
      try {
        if (URL_ATTRIBUTES.includes(name.toLowerCase()) && !shouldSkip(this)) {
          next = rewrite(String(value));
        }
      } catch {
        next = null;
      }
      return originalSetAttribute.call(this, name, next ?? value);
    };
    const observer = new MutationObserver((records) => {
      records.forEach((record) => {
        record.addedNodes.forEach((node) => {
          ...
          rewriteElement(node);
          node.querySelectorAll?.(URL_ATTRIBUTES.map((attr) => `[${attr}]`).join(",")).forEach((child) => rewriteElement(child));
        });
      });
    });
    function rewriteElement(element2) {
      if (shouldSkip(element2)) return;
      URL_ATTRIBUTES.forEach((attribute) => {
        const value = element2.getAttribute(attribute);
        if (!value) return;
        const next = rewrite(value);
        if (next && next !== value) originalSetAttribute.call(element2, attribute, next);
      });
    }
    observer.observe(document.documentElement, { childList: true, subtree: true });
  }

  // src/bridge/google-forms-bridge.ts
  installGoogleFormsProxyRuntime();
  (() => {
    const params2 = new URLSearchParams(window.location.search);
    const root = document.documentElement;
    const roomId = params2.get("roomId") || root.dataset.lowcoderRoomId || "";
    const role = params2.get("role") || root.dataset.lowcoderRole || "driver";
    const editorId = params2.get("editorId") || root.dataset.lowcoderEditorId || "local";
    const collabId = params2.get("collab") || root.dataset.lowcoderCollabId || "";
    const debug = params2.get("debug") === "1";
    const peerId = `${editorId}|${role}|${Math.random().toString(36).slice(2, 10)}`;
    if (!roomId || !collabId) {
      console.error(
        "[google-forms-bridge] Missing roomId/collab. Load the form through /proxy/google-forms and create a session with createGoogleFormsProxySession before Fill Together."
      );
      return;
    }
    if (!window.location.pathname.includes("/proxy/google-forms")) {
      console.error(
        "[google-forms-bridge] Form left the Lowcoder proxy (often after Google sign-in on an /edit URL). Use the published .../viewform responder URL instead of webViewLink."
      );
      return;
    }
    const hocuspocusConfig = window.__LOWCODER_HOCUSPOCUS__ ?? {};
    const hocuspocusUrl = hocuspocusConfig.url || root.dataset.lowcoderHocuspocusUrl || "ws://localhost:3006";
    const hocuspocusToken = hocuspocusConfig.token || root.dataset.lowcoderHocuspocusToken || "";
    ...
    let navigationTimer;
    const doc2 = new Doc();
    const fields = doc2.getMap("fields");
    const state = doc2.getMap("state");
    ...
    function log(...args2) {
      if (debug) console.log("[google-forms-bridge]", role, ...args2);
    }
    ...
    function widgetGroup(widget) {
      return widget.closest('[role="radiogroup"], [role="group"]') || widget.closest('[role="listitem"], [data-params]') || widget.parentElement || widget;
    }
    function widgetIdentity(widget) {
      const group = widgetGroup(widget);
      const question = widget.closest("[data-params]");
      const dataParams = question?.getAttribute("data-params");
      if (dataParams) {
        const groups = Array.from(
          question.querySelectorAll('[role="radiogroup"], [role="group"], [role="listbox"]')
        );
    ...
    function widgetValue(widget) {
      const role2 = widget.getAttribute("role");
      if (role2 === "checkbox") return widget.getAttribute("aria-checked") === "true" ? "1" : "0";
      if (role2 === "radio") {
        const selected2 = widgetGroup(widget).querySelector(
          '[role="radio"][aria-checked="true"]'
        );
        return selected2?.dataset.value || selected2?.getAttribute("aria-label") || "";
      }
      const selected = widget.querySelector('[role="option"][aria-selected="true"]');
    ...
    function controlIdentity(control) {
      ...
      if (name) return `name:${name}`;
      const id2 = control.id?.trim();
      if (id2) return `id:${id2}`;
      const ariaLabel = control.getAttribute("aria-label")?.trim();
      if (ariaLabel) return `aria:${ariaLabel}`;
      const all2 = listControls();
      return `index:${all2.indexOf(control)}`;
    }
    function controlValue(control) {
      ...
      if (control instanceof HTMLInputElement && control.type === "radio") {
        const group = listControls().filter(
          (candidate) => candidate instanceof HTMLInputElement && candidate.type === "radio" && controlIdentity(candidate) === controlIdentity(control)
        );
    ...
    function optionValue(input) {
      return input.closest("[data-value]")?.dataset.value || input.getAttribute("data-value") || input.value;
    }
    function controlKey(control) {
      ...
      if (control instanceof HTMLInputElement && control.type === "checkbox") {
        const peers = listControls().filter(
          (candidate) => candidate instanceof HTMLInputElement && candidate.type === "checkbox" && controlIdentity(candidate) === identity
        );
        const sameValueIndex = peers.filter((candidate) => optionValue(candidate) === optionValue(control)).indexOf(control);
        return `checkbox:${identity}:${optionValue(control)}:${Math.max(0, sameValueIndex)}`;
      }
      const sameIdentity = listControls().filter(
        (candidate) => !(candidate instanceof HTMLInputElement && ["radio", "checkbox"].includes(candidate.type)) && controlIdentity(candidate) === identity
      );
    ...
    function setNativeValue(control, value) {
      if (control instanceof HTMLInputElement && control.type === "radio") {
        const target = listControls().find(
          (candidate) => candidate instanceof HTMLInputElement && candidate.type === "radio" && controlKey(candidate) === controlKey(control) && optionValue(candidate) === value
        );
    ...
    function publishControl(control) {
      ...
      doc2.transact(() => fields.set(key, value), peerId);
      log("published field", key);
    }
    function publishAllControls(onlyMissing = false) {
      doc2.transact(() => {
    ...
    function publishAllWidgets(onlyMissing = false) {
    ...
    function applyWidget(key) {
      ...
      if (role2 === "checkbox") {
        ...
        target = Array.from(widgetGroup(widget).querySelectorAll('[role="radio"]')).find(
          (option) => (option.dataset.value || option.getAttribute("aria-label") || "") === value
        ) ?? null;
      } else {
        target = Array.from(widget.querySelectorAll('[role="option"]')).find(
    ...
    function pageMarker() {
      ...
      const visibleQuestion = Array.from(
        document.querySelectorAll('[role="listitem"], [data-params]')
      ).find((element2) => {
    ...
    function navigationAction(target) {
      ...
      const text2 = (button.textContent || button.value || "").trim().toLowerCase();
      if (jsname === "e19J0b" || /^(back|previous)$/.test(text2)) return "back";
      if (jsname === "OCpkoe" || /^(next|continue)$/.test(text2)) return "next";
      if (jsname === "M2UYVd" || /^(submit|send)$/.test(text2)) return "submit";
      return null;
    }
    function findNavigationButton(action) {
      const jsname = action === "next" ? "OCpkoe" : "e19J0b";
      const byJsName = document.querySelector(`[jsname="${jsname}"]`);
      if (byJsName) return byJsName;
      return Array.from(
        document.querySelectorAll('button, input[type="submit"], [role="button"]')
      ).find((button) => navigationAction(button) === action) ?? null;
    }
    function publishNavigation(action) {
      publishAllControls();
      publishAllWidgets();
      const command = {
      ...
      doc2.transact(() => state.set("navigationJson", JSON.stringify(command)), peerId);
      log("published navigation", command);
    }
    function applyRemoteNavigation() {
      ...
      if (!command.id || command.id === lastNavigationId || command.editorId === peerId || command.action === "submit" || command.fromPage !== pageMarker()) {
        return;
      }
      lastNavigationId = command.id;
    ...
    document.addEventListener(
      ...
      (event) => {
        ...
        const widget = event.target.closest(
          '[role="radio"], [role="checkbox"], [role="listbox"], [role="option"]'
        );
        if (widget) {
          const root2 = widget.getAttribute("role") === "option" ? widget.closest('[role="listbox"]') : widget;
          if (root2) window.setTimeout(() => publishWidget(root2), 0);
        }
      },
      true
    );
    ...
    initTypeformCursorPresence({
      ...
      getCurrentStep: () => {
        ...
        const last2 = marker.split(",").pop();
        return Number(last2) || 0;
      },
