...
exports.isPublishedViewformUrl = isPublishedViewformUrl;
exports.cleanViewformUrl = cleanViewformUrl;
exports.resolveGoogleFormResponderUrl = resolveGoogleFormResponderUrl;
const node_fetch_1 = __importDefault(require("node-fetch"));
const PUBLISHED_VIEWFORM_RE = /\/forms\/d\/e\/[^/]+\/viewform/i;
const DRIVE_FORM_ID_RE = /\/forms\/d\/([^/]+)\/(edit|viewform|preview)/i;
function isPublishedViewformUrl(url) {
    return PUBLISHED_VIEWFORM_RE.test(url.pathname);
}
function cleanViewformUrl(rawUrl) {
    ...
    if (!url.pathname.includes("/viewform")) {
        throw new Error("googleFormUrl must be a published responder URL ending with /viewform. " +
...
function extractViewformFromHtml(html) {
    const patterns = [
...
async function followToPublishedViewform(startUrl) {
    let current = startUrl;
    for (let step = 0; step < 12; step += 1) {
        const response = await (0, node_fetch_1.default)(current, {
            method: "GET",
            redirect: "manual",
            headers: {
                "user-agent": "Mozilla/5.0 (compatible; LowcoderGoogleFormsProxy/1.0; +https://lowcoder.cloud)",
            },
        });
 ...
 */
async function resolveGoogleFormResponderUrl(rawUrl) {
    ...
    throw new Error("Could not resolve a published Google Form responder URL. " +
